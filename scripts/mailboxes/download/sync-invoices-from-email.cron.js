...
async function processAccount(acc) {
    ...
    // Current year/month used to decide which emails should be processed
    const now = new Date();
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth();

    // Ensures the existing current-month folder is deleted only once
    let currentMonthFolderReset = false;

    try {
        for await (let message of client.fetch('1:*', { source: true })) {
            const parsed = await simpleParser(message.source);

            // Use the email's date to determine whether it belongs
            ...
            // Skip emails that are not from the current year/month
            if (
            ...
            // Reset the current month's directory only once,
            // before storing the first current-month PDF
            if (!currentMonthFolderReset) {
                const currentMonthDir = path.join(
                    BASE_DOWNLOAD_PATH,
                    acc.name,
                    year,
                    month
                ...
                if (await fs.pathExists(currentMonthDir)) {
                    console.log(
                        `[+] Existing current month folder found. Removing: ${currentMonthDir}`
                    );

                    await fs.remove(currentMonthDir);
                }

                currentMonthFolderReset = true;
            }
